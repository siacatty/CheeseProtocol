...
namespace CheeseProtocol
{
    public class SupplyAdvancedSettings : CommandAdvancedSettingsBase
    {
        ...
        CheeseRollTrace trace = new CheeseRollTrace("", CheeseCommand.Supply);
        CheeseRollTrace sampleTrace = new CheeseRollTrace("", CheeseCommand.Supply);
        ...
        public override int GetPreviewDirtyHash()
        {
            ...
            dh.AddRange(supplyTierRange);
            dh.AddRange(supplyValueRange);
            dh.AddRange(weaponTierRange);
            dh.AddRange(weaponTechRange);

            dh.Add(allowFoodSupply);
            dh.Add(allowMedSupply);
            dh.Add(allowDrugSupply);
            dh.Add(allowWeaponSupply);
        ...
        public override float DrawResults(Rect rect)
        {
            ...
            if (Widgets.ButtonText(sampledBtn, "미리 뽑아보기"))
            {
                ...
                SupplySpawner.Generate(settings.resultDonation01, sampleTrace);
            ...
            {
                trace.steps.Clear();
                SupplySpawner.Generate(settings.resultDonation01, trace);
